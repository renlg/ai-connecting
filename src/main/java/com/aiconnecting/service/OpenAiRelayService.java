...
public class OpenAiRelayService {
        ...
        while (attempt < RelaySupport.MAX_RETRIES && System.currentTimeMillis() < retryDeadline) {
            ...
            try {
                ...
                if (!httpResponse.isCommitted()) {
                    ...
                    protocolAdapter.writeError(RelayProtocol.OPENAI, httpResponse, 502,
                            SseUtils.clientErrorMessage("所有渠道均不可用: " + lastError,
                                    "All channels are unavailable, please try again later", upstream), upstream);
                }
                return;
            }
            triedChannels.add(channel.getId());
                ...
                if (!httpResponse.isCommitted()) {
                    protocolAdapter.writeError(RelayProtocol.OPENAI, httpResponse, 502,
                            SseUtils.clientErrorMessage("渠道请求失败: " + e.getMessage(),
                                    "Channel request failed: " + e.getMessage(), true), true);
                }
                return;
            }
    ...
    private boolean tryFallbackStream(RelaySupport.RelayContext ctx, String path, String requestBody,
        ...
        var fallback = modelGroupFailoverExecutor.resolveFallbackGroup(ctx.modelConfig().getFallbackGroupId(),
                "text", "admin".equals(ctx.user().getRole()), ctx.userLevel());
